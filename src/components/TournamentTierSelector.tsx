...
export const TournamentTierSelector: React.FC<TournamentTierSelectorProps> = ({
  ...
  const getTierBorderColor = (code: string) => {
    switch (code) {
      case 'G': return 'border-yellow-400';
      case 'H': return 'border-blue-400';
      case 'I': return 'border-green-400';
      case 'K': return 'border-purple-400';
            ...
            <CardContent className="space-y-4">
              ...
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-3 rounded-lg">
                ...
                <div className="grid grid-cols-2 gap-2 mt-2 text-xs">
                  <div className="text-center">
                    ...
                    <div className="text-gray-600">+{tier.elo_points.fourth}</div>
                  </div>
                  <div className="text-center">
              ...
              <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-3 rounded-lg">
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
                    ...
                    <span className="font-semibold text-green-800">Phí đăng ký</span>
                  </div>
                </div>
                <div className="text-center mt-2">
                  <div className="text-sm text-green-700">
                    {tier.entry_fee_range.min.toLocaleString()} - {tier.entry_fee_range.max.toLocaleString()} VNĐ
                  </div>
                </div>
              </div>
              ...
              {(tier.min_rank_requirement || tier.max_rank_requirement) && (
                <div className="bg-gray-50 p-3 rounded-lg">
                  <div className="text-sm font-medium text-gray-700 mb-1">Yêu cầu hạng:</div>
                  <div className="text-xs text-gray-600">
                    {tier.min_rank_requirement && `Từ: ${tier.min_rank_requirement}`}
                    {tier.min_rank_requirement && tier.max_rank_requirement && ' - '}
                    {tier.max_rank_requirement && `Đến: ${tier.max_rank_requirement}`}
                  </div>
                </div>
              )}
            </CardContent>
        ...
        <div className="mt-4 p-4 bg-blue-50 rounded-lg">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-blue-800">
              Đã chọn: {TOURNAMENT_TIERS.find(t => t.code === selectedTier)?.name}
            </span>
          </div>
        </div>
      )}
