...
export const ChallengesList = ({ activeTab }: ChallengesListProps) => {
  const { receivedChallenges, sentChallenges, loadingReceived, loadingSent, respondToChallenge } = useChallenges();

  const handleRespond = async (challengeId: string, status: 'accepted' | 'declined', message?: string) => {
    try {
            ...
            <Card key={challenge.id}>
              ...
              <CardContent className="space-y-4">
                <div className="text-sm text-gray-600">
                  <p><strong>Mức cược:</strong> {challenge.bet_points} điểm</p>
                  {challenge.proposed_datetime && (
                    <p><strong>Thời gian đề xuất:</strong> {formatDateTime(challenge.proposed_datetime)}</p>
                  )}
                  {challenge.club?.name && (
                    <p><strong>Địa điểm:</strong> {challenge.club.name}</p>
                  )}
                  {challenge.message && (
            ...
            <Card key={challenge.id}>
              <CardHeader>
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-3">
                    <Avatar>
                      <AvatarImage src={challenge.challenged_profile?.avatar_url} />
                      <AvatarFallback>
  ...
  if (activeTab === 'history') {
    return (
      <Card>
        <CardContent className="p-6 text-center">
          <p className="text-gray-500">Lịch sử thách đấu sẽ được hiển thị ở đây</p>
        </CardContent>
      </Card>
    );
  }
