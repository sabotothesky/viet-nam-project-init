...
import { runDatabaseConnectionTest, DatabaseConnectionReport } from '@/utils/databaseConnectionTest';
import { checkSupabaseConnection, checkDatabasePermissions, validateEnvironmentVariables } from '@/utils/supabaseHealth';
...
const SystemHealthPage: React.FC = () => {
  ...
  const [databaseReport, setDatabaseReport] = useState<DatabaseConnectionReport | null>(null);
  ...
  const runSystemCheck = async () => {
    ...
    try {
      ...
      // Permissions check
      const permCheck = await checkDatabasePermissions();
      ...
      const errorRate = dbReport.tests.filter(t => !t.success).length / dbReport.tests.length * 100;

      setSystemStatus({
        database: dbCheck.success,
        authentication: dbReport.tests.find(t => t.test === 'Authentication')?.success || false,
        storage: dbReport.tests.find(t => t.test === 'File Storage')?.success || false,
        realtime: dbReport.tests.find(t => t.test === 'Real-time Subscription')?.success || false,
        functions: dbReport.tests.find(t => t.test === 'Database Functions')?.success || false,
              ...
              <div className="flex items-center gap-4">
                {Object.values(systemStatus).every(status => status) ? (
                  ...
                  <Alert variant="destructive">
                    ...
                    <AlertDescription>
                      Some systems are experiencing issues. Check the detailed reports for more information.
                    </AlertDescription>
                  </Alert>
                )}
              </div>
        ...
        <TabsContent value="performance" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <Card>
              ...
              <CardContent>
                <div className={`text-2xl font-bold ${getPerformanceColor(performance.responseTime, 1000)}`}>
            ...
            <Card>
              ...
              <CardContent>
                <div className={`text-2xl font-bold ${getPerformanceColor(performance.errorRate, 5)}`}>
              ...
              <CardContent>
                ...
                <div className="mt-4 p-4 bg-gray-50 rounded-lg">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div>Avg Response: {databaseReport.summary.averageResponseTime}ms</div>
                  </div>
                </div>
              </CardContent>
            </Card>
