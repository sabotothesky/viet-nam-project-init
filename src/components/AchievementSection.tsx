...
const AchievementSection = ({ achievements, user }: AchievementSectionProps) => {
  ...
  const categories = [
    ...
    { id: 'tournament', name: 'Giải đấu', count: achievements.filter(a => a.category === 'tournament').length },
    { id: 'streak', name: 'Chuỗi thắng', count: achievements.filter(a => a.category === 'streak').length },
    { id: 'skill', name: 'Kỹ năng', count: achievements.filter(a => a.category === 'skill').length },
    { id: 'milestone', name: 'Cột mốc', count: achievements.filter(a => a.category === 'milestone').length },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-center mb-6">
        <h2 className="heading-secondary text-gray-900 mb-2">Thành tích của {user.name}</h2>
        <div className="flex justify-center space-x-6">
          <div className="text-center">
            <p className="text-2xl font-bold text-primary-blue">{user.total_achievements}</p>
            <p className="text-xs text-comment-gray">Tổng thành tích</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-bold text-gold">{user.rare_achievements}</p>
          ...
          <div className="flex space-x-4 overflow-x-auto scrollbar-hide pb-2">
            {featuredAchievements.map((achievement) => (
              <div key={achievement.id} className="flex-shrink-0 text-center">
                <div className={`w-20 h-20 rounded-full p-1 ${getRarityStyle(achievement.rarity)}`}>
          ...
          <Card key={achievement.id} className="hover:shadow-md transition-shadow">
            <CardContent className="p-4">
              <div className="flex items-start space-x-4">
                {/* Achievement Icon */}
                <div className={`w-16 h-16 rounded-full flex items-center justify-center ${getRarityStyle(achievement.rarity)}`}>
                  {getAchievementIcon(achievement.icon)}
                </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <div className="flex flex-col items-end space-y-1">
                      <Badge className={getRarityStyle(achievement.rarity)} variant="secondary">
                        {achievement.rarity.toUpperCase()}
                      </Badge>
                      <Badge className={getCategoryStyle(achievement.category)} variant="secondary">
      ...
      {filteredAchievements.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-500 font-medium">Chưa có thành tích nào trong danh mục này</p>
        </div>
      )}
    </div>
  );
