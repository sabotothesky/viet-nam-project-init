...
export const quickDatabaseCheck = async () => {
  ...
  try {
    ...
    const importantTables = ['profiles', 'clubs', 'tournaments', 'challenges', 'matches', 'notifications'];
    results.tables = importantTables;
    console.log(`✅ Sử dụng danh sách bảng mặc định: ${importantTables.length} bảng`);
    ...
    for (const table of importantTables) {
      try {
        ...
        if (!error && data) {
          ...
          console.log(`⚠️ Bảng ${table}: ${error?.message || 'Không có dữ liệu'}`);
        }
      } catch (error: any) {
    ...
    if (authError) {
      ...
      console.log(`✅ Authentication: ${user ? 'Đã đăng nhập' : 'Chưa đăng nhập'}`);
    }

    // 6. Tóm tắt kết quả
    console.log('\n📊 TÓM TẮT KIỂM TRA DATABASE:');
    console.log('================================');
    console.log(`🔗 Kết nối: ${results.connection ? '✅ Thành công' : '❌ Thất bại'}`);
    ...
    if (!results.connection) {
      console.log('  - ❌ Database không thể kết nối. Cần kiểm tra lại cấu hình.');
    } else if (results.hasData) {
      console.log('  - ✅ Database có dữ liệu. Có thể sử dụng database hiện tại.');
...
export const runQuickCheck = () => {
  ...
  quickDatabaseCheck().then(results => {
