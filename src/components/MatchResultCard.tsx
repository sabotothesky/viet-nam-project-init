...
import { Heart, MessageCircle, Share2, Trophy, Target } from 'lucide-react';
      ...
      <CardContent className="p-4">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-2">
            ...
            {match.tournament ? (
              <div>
                <p className="text-sm font-medium text-gray-900">{match.tournament.name}</p>
                <p className="text-xs text-comment-gray">{match.tournament.round}</p>
              </div>
            ) : (
              <p className="text-sm font-medium text-gray-900">Trận đấu thường</p>
            )}
          </div>
          <div className="text-right">
            <p className="text-xs text-comment-gray">{formatDate(match.match_date)}</p>
            <p className="text-xs text-comment-gray">Thời gian: {match.duration}</p>
          </div>
        </div>
        ...
        <div className="bg-tertiary rounded-lg p-4 mb-4">
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            <div className={`px-6 py-3 rounded-lg text-center ${getWinnerStyle(match.player1.id)}`}>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className={`px-6 py-3 rounded-lg text-center ${getWinnerStyle(match.player2.id)}`}>
        ...
        <div className="text-center mb-4">
          <Badge className="bg-primary-blue text-white px-4 py-2">
            🏆 Chiến thắng thuộc về {match.winner_id === match.player1.id ? match.player1.name : match.player2.name}
          </Badge>
        </div>
            ...
            {showStats && (
              <div className="mt-3 grid grid-cols-2 gap-4 p-3 bg-secondary rounded-lg">
                <div className="text-center">
                  ...
                  <p className="text-lg font-bold text-primary-blue">{match.stats.longest_run_p1 || 0}</p>
                </div>
                <div className="text-center">
                  <p className="text-xs text-comment-gray">Run dài nhất P2</p>
                  <p className="text-lg font-bold text-primary-blue">{match.stats.longest_run_p2 || 0}</p>
                </div>
                <div className="text-center">
                  <p className="text-xs text-comment-gray">Tổng shots P1</p>
                  <p className="text-lg font-bold text-primary-green">{match.stats.total_shots_p1 || 0}</p>
                </div>
                <div className="text-center">
                  <p className="text-xs text-comment-gray">Tổng shots P2</p>
                  <p className="text-lg font-bold text-primary-green">{match.stats.total_shots_p2 || 0}</p>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-gray-100">
          ...
          >
            <Heart className={`w-5 h-5 ${match.isLiked ? 'fill-current' : ''}`} />
