...
import { QrCode, Users, Trophy, Clock, CheckCircle, XCircle } from 'lucide-react';
...
export const QRSystemPage: React.FC = () => {
  ...
  const handleCreateTableQR = async (clubId: string, tableNumber: number, tableName: string) => {
  ...
  const getGameTypeLabel = (gameType: string) => {
    switch (gameType) {
      ...
      case 'song_to': return 'Song Tô';
      default: return gameType;
    }
  };

  const filteredRankings = selectedRank === 'all' 
    ? playerRankings 
            ...
            <Card>
              ...
              <CardContent>
                <div className="flex gap-2">
                  ...
                  <Button variant="outline" onClick={() => setScannedTable(null)}>
                    Xóa bàn hiện tại
                  </Button>
                </div>
              </CardContent>
        ...
        <TabsContent value="rankings" className="space-y-6">
          <Card>
            ...
            <CardContent>
              {/* Rank Filter */}
              <div className="mb-4">
                <label className="block text-sm font-medium mb-2">Lọc theo hạng:</label>
                <select
                  value={selectedRank}
                  ...
                  <div key={player.id} className="border rounded-lg p-4">
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium">{player.balls_consistent}</span>
                      </div>
                ...
                <div className="space-y-4">
                  {userClubs.map((club) => (
                    <div key={club.id} className="border rounded-lg p-4">
                      ...
                      <p className="text-sm text-gray-600 mb-3">{club.address}</p>
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center gap-2">
                          ...
                          <Button
                            onClick={() => {
                              const tableNumber = (document.getElementById(`table-${club.id}`) as HTMLInputElement)?.value;
                              const tableName = (document.getElementById(`name-${club.id}`) as HTMLInputElement)?.value;
                              if (tableNumber && tableName) {
                                handleCreateTableQR(club.id, parseInt(tableNumber), tableName);
                              }
                            }}
                          >
                            Tạo QR
