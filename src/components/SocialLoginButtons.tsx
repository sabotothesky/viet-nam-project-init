      ...
      if (error) {
        ...
        if (error.message.includes('provider is not enabled')) {
          toast({
            ...
            description: "Đăng nhập Google đang được cấu hình. Vui lòng sử dụng email để đăng ký/đăng nhập.",
            variant: "destructive",
          });
        } else {
          toast({
            title: "Lỗi đăng nhập Google",
            description: error.message || "Không thể đăng nhập với Google. Vui lòng thử lại.",
            variant: "destructive",
          });
        }
      } else {
    ...
    try {
      ...
      if (error) {
        ...
        if (error.message.includes('provider is not enabled')) {
          toast({
            title: "Chức năng chưa khả dụng",
            description: "Đăng nhập Facebook đang được cấu hình. Vui lòng sử dụng email để đăng ký/đăng nhập.",
            variant: "destructive",
          });
        } else {
          toast({
            title: "Lỗi đăng nhập Facebook",
            description: error.message || "Không thể đăng nhập với Facebook. Vui lòng thử lại.",
            variant: "destructive",
          });
        }
      } else {
    ...
    <div className="space-y-3">
      ...
      <div className="text-center">
        <p className="text-xs text-gray-500 mt-2">
          💡 <strong>Lưu ý:</strong> Tính năng đăng nhập mạng xã hội đang được cấu hình.
          <br />
