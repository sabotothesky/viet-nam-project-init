-- Deploy Current Season Data (2025-S2)
-- This script adds the current season data and related functions

-- 1. Add current season data
...
-- 2. Create current_season table if not exists
...
ON CONFLICT (season_name, season_year) DO UPDATE SET
...
BEGIN
  ...
  ORDER BY sh.ranking_points DESC, sh.final_rank ASC
  LIMIT 1;
END;
$$ LANGUAGE plpgsql;

-- 5. Create season_stats view if not exists
...
SELECT 
  ...
  ROUND(AVG(ranking_points)) as average_points,
  COUNT(DISTINCT final_rank) as total_ranks
FROM season_history
...
-- 6. Verify deployment
SELECT 
  'Current Season Deployment Complete' as status,
  COUNT(*) as total_players_added,
...
-- 7. Show current season info
SELECT 
  'Current Season Info' as info_type,
  season_name,
  season_year,
  start_date,
  end_date,
...
-- 8. Show season progress
SELECT 
  'Season Progress' as info_type,
  total_days,
  days_elapsed,
  days_remaining,
  progress_percentage
FROM get_season_progress('S2', 2025); 