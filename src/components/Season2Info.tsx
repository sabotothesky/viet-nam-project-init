...
import { Trophy, Calendar, Users, Award, CheckCircle, XCircle } from 'lucide-react';
import { toast } from '../hooks/use-toast';

export const Season2Info: React.FC = () => {
  const { seasonInfo, prizes, loading, checkSeason2Eligibility, registerForSeason2 } = useSeason2();
  const { user } = useAuth();
    ...
    <div className="space-y-6">
      ...
      <Card className="bg-gradient-to-r from-blue-600 to-purple-600 text-white">
        <CardHeader>
          ...
          <CardDescription className="text-blue-100">
            Thời gian: {new Date(seasonInfo.start_date).toLocaleDateString('vi-VN')} - {new Date(seasonInfo.end_date).toLocaleDateString('vi-VN')}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="text-center">
              <div className="text-2xl font-bold">{seasonInfo.total_participants}</div>
              <div className="text-sm text-blue-100">Người tham gia</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold">{seasonInfo.total_matches}</div>
              <div className="text-sm text-blue-100">Trận đấu</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold">{seasonInfo.registration_fee.toLocaleString()}đ</div>
              <div className="text-sm text-blue-100">Phí đăng ký</div>
            </div>
            <div className="text-center">
              <Badge variant="secondary" className="text-blue-600">
                {seasonInfo.status === 'active' ? 'Đang diễn ra' : 'Đã kết thúc'}
              </Badge>
            </div>
          </div>
        </CardContent>
            ...
            {checkingEligibility ? (
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className={eligibility.eligible ? "text-green-600" : "text-red-600"}>
                    {eligibility.eligible ? "Đủ điều kiện tham gia" : "Chưa đủ điều kiện"}
                  </span>
                </div>
                
                {eligibility.currentRank && (
                  <div className="text-sm text-gray-600">
                    Hạng hiện tại: <Badge variant="outline">{eligibility.currentRank}</Badge>
                  </div>
                )}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Tổng giá trị giải thưởng: {prizes.reduce((sum, prize) => sum + prize.prize_value, 0).toLocaleString()}đ
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-3">
