...
const EnhancedPlayerCard = ({ player, index = 0, onChallenge }: PlayerCardProps) => {
      ...
      <div className="h-1/2 relative">
        ...
        <div className="absolute bottom-4 left-4 text-white">
          <h2 className="text-2xl font-bold">
            {player.full_name}{player.age ? `, ${player.age}` : ''}
          </h2>
      ...
      <div className="h-1/2 p-4 space-y-4">
        ...
        <div className="bg-blue-50 rounded-lg p-3">
          ...
          {player.preferred_club?.address && (
            <p className="text-xs text-blue-600">{player.preferred_club.address}</p>
          )}
        </div>
        ...
        <div className="bg-green-50 rounded-lg p-3">
          ...
          <div className="flex flex-wrap gap-1">
            {player.preferred_play_times?.map(time => (
              <Badge key={time} variant="secondary" className="bg-green-200 text-green-800 text-xs">
                {getTimeLabel(time)}
              </Badge>
            )) || (
        ...
        <div className="bg-purple-50 rounded-lg p-3">
          <div className="flex items-center space-x-2 mb-2">
            ...
            <span className="font-semibold text-purple-800">Mức cược (Điểm Ranking)</span>
          </div>
        ...
        <div className="grid grid-cols-3 gap-3 text-center">
          ...
          <div>
            <div className="text-lg font-bold text-orange-600">{player.matches_won}</div>
