...
interface Transaction {
  id: string;
  type: 'deposit' | 'withdraw' | 'transfer' | 'match_win' | 'match_loss' | 'challenge' | 'refund' | 'bonus';
  amount: number;
  description: string;
...
export const TransactionHistory: React.FC<TransactionHistoryProps> = ({ userId }) => {
  ...
  const filters = [
    ...
    { id: 'transfer', name: 'Chuyển điểm', icon: <ArrowUpRight className="h-4 w-4" /> },
    { id: 'match_win', name: 'Thắng trận', icon: <ArrowUpRight className="h-4 w-4" /> },
    { id: 'challenge', name: 'Thách đấu', icon: <ArrowDownRight className="h-4 w-4" /> }
  ];
  ...
  const filteredTransactions = transactions
    .filter(transaction => {
      ...
      const matchesStatusFilter = selectedStatus === 'all' || transaction.status === selectedStatus;
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-medium">{getTransactionTypeName(transaction.type)}</span>
                  ...
                  <div className="flex items-center gap-4 text-xs text-gray-500">
                    <div className="flex items-center gap-1">
                      ...
                      <span>{transaction.created_at.toLocaleDateString('vi-VN')}</span>
                    </div>
