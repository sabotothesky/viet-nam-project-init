...
export const EloStatistics: React.FC<EloStatisticsProps> = ({ player, className }) => {
  const [activeTab, setActiveTab] = useState<'overview' | 'progression' | 'performance' | 'comparison'>('overview');
  ...
  const getFormIcon = (form: number) => {
    ...
    if (form > -50) return <TrendingDownIcon className="h-4 w-4 text-orange-500" />;
  ...
  const getVolatilityIcon = (volatility: number) => {
    if (volatility < 30) return <TargetIcon className="h-4 w-4 text-green-500" />;
  ...
  const efficiency = calculateEloEfficiency(player.current_rating, 1000, player.matches_played);
                ...
                <Card className="border-blue-200 bg-blue-50">
                  <CardContent className="p-4">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="font-medium text-blue-800">Current Rank</span>
                    </div>
                ...
                <Card className="border-green-200 bg-green-50">
                  <CardContent className="p-4">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="font-medium text-green-800">Win Rate</span>
                    </div>
                ...
                <Card className="border-purple-200 bg-purple-50">
                  <CardContent className="p-4">
                    ...
                    <div className="text-sm text-purple-600">
                      {player.current_streak > 0 ? 'Winning streak' : 'No active streak'}
                    </div>
                  </CardContent>
                </Card>
              </div>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <div className={`font-semibold ${getFormColor(player.recent_form || 0)}`}>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className={`font-semibold ${getConsistencyColor(player.consistency_score || 50)}`}>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className={`font-semibold ${getVolatilityColor(player.rating_volatility || 0)}`}>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold">
                        {(player.highest_rating || player.current_rating) - (player.lowest_rating || player.current_rating)}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span>ELO Efficiency:</span>
                      <span className="font-semibold text-blue-600">
                        {efficiency > 0 ? '+' : ''}{efficiency} pts/match
                      </span>
                    </div>
                    <div className="flex justify-between">
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="text-center p-4 bg-blue-50 rounded-lg">
                      <div className="text-sm text-blue-600 mb-1">Current Rank</div>
                      <Badge className={getRankColor(player.current_rating)}>
                    ...
                    <div className="text-center p-4 bg-green-50 rounded-lg">
                      <div className="text-sm text-green-600 mb-1">Next Rank</div>
                      <Badge className={getRankColor(getRatingFromRank(progression.nextRank))}>
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-semibold">
                        {Math.round((player.current_rating - getRatingFromRank(progression.currentRank)) / 200 * 100)}%
                      </span>
                    </div>
                    <Progress 
                      value={(player.current_rating - getRatingFromRank(progression.currentRank)) / 200 * 100} 
                      className="h-3"
                    />
                    <div className="text-sm text-gray-600">
                      Estimated {progression.estimatedMatches} matches to reach {progression.nextRank}
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                      {['E', 'E+', 'D', 'D+', 'C', 'C+', 'B', 'B+', 'A', 'A+', 'G', 'G+', 'S', 'S+'].map((rank) => {
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="flex justify-between">
                      <span>Wins:</span>
                      <span className="font-semibold text-green-600">{player.wins || 0}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Losses:</span>
                      <span className="font-semibold text-red-600">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold">
                        {player.current_streak > 0 ? '+' : ''}{player.current_streak}
                      </span>
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    <div className="space-y-2">
                      <div className="flex justify-between">
                        <span>Recent Form:</span>
                        <span className={`font-semibold ${getFormColor(player.recent_form || 0)}`}>
                          {player.recent_form || 0}
                        </span>
                      </div>
                      <div className="text-xs text-gray-600">
                        {player.recent_form > 50 ? '🔥 Excellent form' : 
                         player.recent_form > 20 ? '📈 Good form' : 
                         player.recent_form > -20 ? '➡️ Stable form' : 
                         player.recent_form > -50 ? '📉 Poor form' : '❄️ Cold streak'}
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between">
                        <span>Consistency:</span>
                        <span className={`font-semibold ${getConsistencyColor(player.consistency_score || 50)}`}>
                          {player.consistency_score || 50}%
                        </span>
                      </div>
                      <div className="text-xs text-gray-600">
                        {player.consistency_score > 80 ? '🛡️ Very stable performance' : 
                         player.consistency_score > 60 ? '📊 Stable performance' : 
                         player.consistency_score > 40 ? '📈 Variable performance' : '📉 Volatile performance'}
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between">
                        ...
                        <span className={`font-semibold ${getVolatilityColor(player.rating_volatility || 0)}`}>
                          {player.rating_volatility || 0}
                        </span>
                      </div>
                      <div className="text-xs text-gray-600">
                        {player.rating_volatility < 30 ? '🎯 Very predictable' : 
                         player.rating_volatility < 60 ? '📊 Moderately predictable' : 
                         player.rating_volatility < 100 ? '⚡ Somewhat unpredictable' : '🎲 Highly unpredictable'}
                      </div>
                    </div>
                  </CardContent>
                </Card>
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="text-center p-4 bg-green-50 rounded-lg">
                      ...
                      <div className="text-2xl font-bold text-green-600">{player.current_rating}</div>
                      <Badge className={getRankColor(player.current_rating)}>
                        {getRankName(player.current_rating)}
                      </Badge>
                    </div>
                    <div className="text-center p-4 bg-blue-50 rounded-lg">
                      <div className="text-sm text-blue-600 mb-1">Average Opponent</div>
                      <div className="text-2xl font-bold text-blue-600">
                        {player.average_opponent_rating || 'N/A'}
                      </div>
                      <div className="text-sm text-gray-600">
                        {player.average_opponent_rating ? 
                          (player.current_rating > player.average_opponent_rating ? 'You are stronger' : 'You are weaker') 
                          : 'No data'
                        }
                      </div>
                    </div>
                    <div className="text-center p-4 bg-purple-50 rounded-lg">
                      <div className="text-sm text-purple-600 mb-1">Rating Difference</div>
                      <div className="text-2xl font-bold text-purple-600">
                        {player.average_opponent_rating ? 
                          (player.current_rating - player.average_opponent_rating > 0 ? '+' : '') +
                          (player.current_rating - player.average_opponent_rating)
                          : 'N/A'
                        }
                      </div>
                      <div className="text-sm text-gray-600">
                        {player.average_opponent_rating ? 
                          Math.abs(player.current_rating - player.average_opponent_rating) + ' points'
                          : 'No data'
                        }
                      </div>
                    </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between">
                        <span>ELO Efficiency:</span>
                        <span className={`font-semibold ${efficiency > 0 ? 'text-green-600' : 'text-red-600'}`}>
                          {efficiency > 0 ? '+' : ''}{efficiency} pts/match
                        </span>
                      </div>
                      <div className="flex justify-between">
                        ...
                        <span className="font-semibold">
                          {player.matches_played > 0 ? 
                            ((player.current_rating - 1000) / player.matches_played).toFixed(1) + ' pts/match'
                            : 'N/A'
                          }
                        </span>
                      </div>
                      <div className="flex justify-between">
                        ...
                        <span className="font-semibold">
                          {player.win_rate && player.win_rate > 50 ? 'Above average' : 'Below average'}
                        </span>
                      </div>
                    </div>
                  </div>
