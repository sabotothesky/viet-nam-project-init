...
import { checkCurrentDatabaseStatus, DatabaseStatus } from '@/utils/databaseStatusChecker';
import { DATABASE_SETUP_GUIDE, getSetupRecommendation } from '@/utils/databaseSetupGuide';
...
const DatabaseSetupWizard: React.FC = () => {
  ...
  const checkDatabase = async () => {
    ...
    try {
      ...
      const recommendation = getSetupRecommendation(dbStatus.hasData, dbStatus.isAccessible);
      ...
      <Card>
        ...
        <CardContent>
          <Tabs value={selectedOption} onValueChange={setSelectedOption}>
            ...
            {Object.entries(DATABASE_SETUP_GUIDE.options).map(([key, option]) => (
              <TabsContent key={key} value={key} className="space-y-4">
                <div className="p-4 border rounded-lg">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                      <h4 className="font-medium text-red-600 mb-2">Nhược điểm:</h4>
                      <ul className="space-y-1">
                  ...
                  <div className="mt-4 flex gap-2">
                    <Button variant="default">
                      {key === 'useExisting' && <Settings className="h-4 w-4 mr-2" />}
                      {key === 'createNew' && <Plus className="h-4 w-4 mr-2" />}
                      {key === 'backupExisting' && <Download className="h-4 w-4 mr-2" />}
                      Thực hiện
                    </Button>
