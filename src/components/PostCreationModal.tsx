...
const PostCreationModal = ({ isOpen, onClose, onSubmit }: PostCreationModalProps) => {
  const { user } = useAuth();
  const [content, setContent] = useState('');
  const [postType, setPostType] = useState<'general' | 'match_result' | 'achievement' | 'tournament_win'>('general');
  ...
  const getPostTypeIcon = () => {
    switch (postType) {
      case 'match_result': return <Trophy className="w-5 h-5" />;
      case 'achievement': return <Star className="w-5 h-5" />;
      case 'tournament_win': return <Zap className="w-5 h-5" />;
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <Badge variant="secondary" className="text-xs">{mockUser.rank}</Badge>
            </div>
          </div>
          ...
          <Select value={postType} onValueChange={(value: string) => setPostType(value)}>
            ...
            <SelectContent>
              ...
              <SelectItem value="tournament_win">Chiến thắng giải đấu</SelectItem>
            </SelectContent>
          </Select>
          ...
          {postType === 'match_result' && (
            <div className="space-y-3 p-4 bg-blue-50 rounded-lg">
              ...
              <Select value={matchData.result} onValueChange={(value: string) => setMatchData(prev => ({ ...prev, result: value }))}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
