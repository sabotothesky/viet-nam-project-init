...
export const useLeaderboard = () => {
  ...
  const fetchLeaderboard = useCallback(
    async (pageNum: number = 1, limit: number = 50) => {
      try {
        setLoading(true);
        setError(null);
  ...
  const getFilteredEntries = useCallback(() => {
    ...
    if (filters.min_matches) {
      filtered = filtered.filter(
        entry => entry.total_matches >= filters.min_matches!
      );
    }

    // Filter by ELO range
    if (filters.min_elo) {
      filtered = filtered.filter(entry => entry.elo_rating >= filters.min_elo!);
    }

    if (filters.max_elo) {
      filtered = filtered.filter(entry => entry.elo_rating <= filters.max_elo!);
    }
  ...
  // Get players by rank
  const getPlayersByRank = useCallback(
    (rank: string) => {
  ...
  const getRisingStars = useCallback(
    (limit: number = 10) => {
      return entries
  ...
  const getActivePlayers = useCallback(
    (days: number = 7) => {
      ...
      return entries.filter(
        entry => entry.last_match_date && entry.last_match_date > cutoffDate
      );
    },
    [entries]
  );
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('profiles')
        .select(
          'full_name, current_rank, ranking_points, wins, losses, total_matches, location'
        )
