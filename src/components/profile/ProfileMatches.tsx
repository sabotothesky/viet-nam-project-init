...
export const ProfileMatches: React.FC<ProfileMatchesProps> = ({ userId }) => {
  ...
  const filters = [
    ...
    { id: 'challenge', name: 'Thách đấu', icon: <Target className="h-4 w-4" /> },
  ...
  const filteredMatches = matches
    .filter(match => {
      const matchesSearch = match.opponent.username.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            ...
            <div className="text-center">
              <div className={`text-2xl font-bold ${stats.totalRatingChange >= 0 ? 'text-green-600' : 'text-red-600'}`}>
        ...
        <CardContent className="pt-6">
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="flex items-center gap-2">
              <Button
                variant="outline"
                onClick={() => setSortBy(sortBy === 'date' ? 'rating_change' : 'date')}
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            Lịch sử trận đấu ({filteredMatches.length})
          </CardTitle>
        </CardHeader>
        <CardContent>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 mb-2">
                    <Avatar className="h-8 w-8">
                      ...
                      <AvatarFallback>{match.opponent.username[0]}</AvatarFallback>
                    </Avatar>
                    <div>
                      <div className="font-medium">{match.opponent.username}</div>
                      <div className="text-sm text-gray-600">Hạng {match.opponent.rank}</div>
                    </div>
                  </div>
                ...
                <div className="text-right">
                  ...
                  <div className={`text-sm font-medium ${
                    match.rating_change > 0 ? 'text-green-600' : 
                    match.rating_change < 0 ? 'text-red-600' : 'text-gray-600'
                  }`}>
                    {match.rating_change > 0 ? '+' : ''}{match.rating_change}
                  </div>
                  <Badge className={`text-xs mt-1 ${getMatchTypeColor(match.match_type)}`}>
