...
import { CheckCircle, XCircle, Loader2, Database, Shield, User, Bell, CreditCard, Link } from 'lucide-react';
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold">Database CRUD & Authentication Testing</h1>
        <div className="flex space-x-2">
          <Button onClick={runAuthTests} disabled={isRunning}>
            {isRunning ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <User className="w-4 h-4 mr-2" />}
            Test Auth
          </Button>
          <Button onClick={runDatabaseTests} disabled={isRunning}>
            {isRunning ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Database className="w-4 h-4 mr-2" />}
            Test Database
          </Button>
      ...
      {user ? (
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center space-x-3">
              <XCircle className="h-5 w-5 text-red-500" />
              <p className="font-medium">Not authenticated - please login to run tests</p>
            </div>
          </CardContent>
        </Card>
      )}
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center space-x-2">
                ...
                <Button onClick={runNotificationTests} disabled={isRunning} size="sm">
                  Run Tests
                </Button>
              </CardTitle>
            </CardHeader>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center space-x-2">
                ...
                <Button onClick={runPaymentTests} disabled={isRunning} size="sm">
                  Run Tests
                </Button>
              </CardTitle>
            </CardHeader>
