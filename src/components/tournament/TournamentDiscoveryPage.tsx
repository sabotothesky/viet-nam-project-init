...
import { Calendar, MapPin, Users, Trophy, Target, Wifi, WifiOff, Clock } from 'lucide-react';
      ...
      <div className="bg-gray-50 min-h-screen">
        ...
        <div className="bg-white mx-4 mt-4 rounded-xl p-4 shadow-sm">
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              <div className="text-2xl font-bold text-blue-600">{displayTournaments.length}</div>
              <div className="text-xs text-gray-500">Giải đấu</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-green-600">
                {displayTournaments.reduce((sum, t) => sum + (t.total_prize_pool || 0), 0).toLocaleString()}
              </div>
              <div className="text-xs text-gray-500">Tổng giải thưởng</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-purple-600">
                {displayTournaments.filter(t => (t.recommendation_score || 0) > 300).length}
              </div>
        ...
        <div className="p-4 space-y-4">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 font-medium">Không có giải đấu nào phù hợp</p>
              <p className="text-gray-400 text-sm mt-2">Hãy thử thay đổi bộ lọc hoặc quay lại sau</p>
            </div>
          )}
        </div>
      </div>
