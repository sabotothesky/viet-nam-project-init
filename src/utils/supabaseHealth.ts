...
export const checkDatabasePermissions = async (): Promise<HealthCheckResult> => {
  try {
    ...
    if (readError) {
      return { success: false, error: `Read permission failed: ${readError.message}` };
    }
    ...
    if (user) {
      ...
      if (writeError) {
        return { success: false, error: `Write permission failed: ${writeError.message}` };
      }
    }
