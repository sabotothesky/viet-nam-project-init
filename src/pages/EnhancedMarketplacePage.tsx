...
import { 
  Search, Filter, Heart, ShoppingCart, Star, MapPin, 
  Truck, MessageCircle, Shield, Award, Clock, Eye,
...
const EnhancedMarketplacePage = () => {
  ...
  const getConditionColor = (condition) => {
    const conditionObj = conditions.find(c => c.id === condition.toLowerCase().replace(' ', '_'));
  ...
  const getItemTags = (item) => {
    ...
    // Brand item
    if (item.brand && ['Predator', 'Diamond', 'Mezz', 'McDermott'].includes(item.brand)) {
      tags.push({
        icon: '⭐',
        text: 'Hãng nổi tiếng',
          ...
          {itemTags.length > 0 && (
            <div className="flex flex-wrap gap-1 mb-3">
              {itemTags.slice(0, 2).map((tag, index) => (
                <span key={index} className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${tag.color}`}>
                  <span className="mr-1">{tag.icon}</span>
                  {tag.text}
                </span>
              ))}
              {itemTags.length > 2 && (
                <span className="text-xs text-gray-500">+{itemTags.length - 2}</span>
              )}
            </div>
          )}
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Chợ Bida Online</h1>
        <p className="text-gray-600">Mua bán thiết bị bida chất lượng cao với hệ thống uy tín</p>
      </div>
      ...
      <div className="mb-6 space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
          ...
          <Select value={filters.province_id} onValueChange={(value) => setFilters(prev => ({ ...prev, province_id: value }))}>
            <SelectTrigger>
          ...
          <div className="flex items-center space-x-6">
            ...
            <div className="flex items-center space-x-4 text-sm text-gray-500">
              <span className="flex items-center">
                ...
                {filteredItems.filter(item => item.seller?.club_id).length} từ người bán uy tín
              </span>
              <span className="flex items-center">
                <Truck className="w-4 h-4 mr-1 text-green-600" />
                {filteredItems.filter(item => item.shipping_available).length} hỗ trợ ship
              </span>
            </div>
          </div>
