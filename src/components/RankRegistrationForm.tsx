...
const RankRegistrationForm = ({ onSuccess }: RankRegistrationFormProps) => {
  ...
  const { createRankRequest, rankRequests, getStatusText, getStatusColor } = useRankRequests();
  ...
  const onSubmit = async (formData: any) => {
    ...
    try {
      ...
      reset();
      toast.success('Đã gửi yêu cầu rank thành công! CLB sẽ xem xét và phản hồi sớm.');
        ...
        <CardContent>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div>
              ...
              {errors.club_id && (
                <p className="mt-1 text-sm text-red-600">{String(errors.club_id.message)}</p>
              )}
            </div>
            ...
            <div>
              ...
              <Select onValueChange={(value) => setValue('requested_rank', value)}>
                <SelectTrigger>
                  <SelectValue placeholder="Chọn rank bạn muốn đăng ký" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="500">500 ELO - Người mới bắt đầu</SelectItem>
                  <SelectItem value="1000">1000 ELO - Người chơi cơ bản</SelectItem>
                  <SelectItem value="1500">1500 ELO - Người chơi trung bình</SelectItem>
                  <SelectItem value="2000">2000 ELO - Người chơi khá</SelectItem>
                  <SelectItem value="2500">2500 ELO - Người chơi giỏi</SelectItem>
                  <SelectItem value="3000">3000 ELO - Cao thủ</SelectItem>
                </SelectContent>
              </Select>
              {errors.requested_rank && (
                <p className="mt-1 text-sm text-red-600">{String(errors.requested_rank.message)}</p>
              )}
              
              {requestedRank && (
                <div className="mt-2 p-3 bg-blue-50 rounded-lg">
                  <div className="text-sm text-blue-800">
                    <strong>Mô tả rank:</strong> {getRankDescription(parseInt(requestedRank))}
                  </div>
                </div>
              )}
            </div>
            ...
            <Alert>
              ...
              <AlertDescription>
                <strong>Lưu ý:</strong> Việc đăng ký rank sai có thể dẫn đến việc tài khoản bị cấm. 
                Hãy đăng ký rank phù hợp với khả năng thực tế của bạn.
              </AlertDescription>
            </Alert>
