...
export const Season2Page: React.FC = () => {
  const { seasonInfo, prizes, getSeason2Leaderboard, getUserSeason2Stats, loading } = useSeason2();
  const { user } = useAuth();
  ...
  const getRankBadge = (rank: number) => {
    if (rank === 1) return <Badge className="bg-yellow-500 text-white">🥇 Top 1</Badge>;
    if (rank === 2) return <Badge className="bg-gray-400 text-white">🥈 Top 2</Badge>;
    if (rank === 3) return <Badge className="bg-orange-500 text-white">🥉 Top 3</Badge>;
    if (rank <= 10) return <Badge className="bg-blue-500 text-white">Top {rank}</Badge>;
    if (rank <= 20) return <Badge className="bg-green-500 text-white">Top {rank}</Badge>;
    if (rank <= 40) return <Badge className="bg-purple-500 text-white">Top {rank}</Badge>;
    return <Badge variant="outline">#{rank}</Badge>;
  };

  const getPrizeForRank = (rank: number) => {
    return prizes.find(prize => rank >= prize.rank_min && rank <= prize.rank_max);
  };
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <div className="text-sm text-gray-600">
                                {entry.user?.nickname && `@${entry.user.nickname}`} • Hạng {entry.user?.current_rank}
                              </div>
                            </div>
                          </div>
                        </div>
        ...
        <TabsContent value="prizes" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Tổng giá trị: {prizes.reduce((sum, prize) => sum + prize.prize_value, 0).toLocaleString()}đ
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {prizes.map((prize) => (
                  <div key={prize.id} className="border rounded-lg p-6 hover:shadow-lg transition-shadow">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-3">
                        <div className={`w-12 h-12 rounded-full flex items-center justify-center text-white font-bold ${
                          prize.rank_min === 1 ? 'bg-gradient-to-r from-yellow-400 to-yellow-600' :
                          prize.rank_min <= 3 ? 'bg-gradient-to-r from-orange-400 to-orange-600' :
                          prize.rank_min <= 10 ? 'bg-gradient-to-r from-blue-400 to-blue-600' :
                        ...
                        <div>
                          <h3 className="font-bold text-lg">
                            Top {prize.rank_min}{prize.rank_max !== prize.rank_min ? ` - ${prize.rank_max}` : ''}
                          </h3>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center justify-between">
                          <span className="text-sm text-gray-600">Giá trị giải thưởng:</span>
                        ...
                        {prize.member_months > 0 && (
                          <div className="flex items-center justify-between">
                            <span className="text-sm text-gray-600">Thẻ Member:</span>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div className="text-center p-4 bg-orange-50 rounded-lg">
                      <div className="text-2xl font-bold text-orange-600">
                        {leaderboard.findIndex(entry => entry.user_id === user.id) + 1 || 'N/A'}
                      </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="text-center p-4 bg-gray-50 rounded-lg">
                  <div className="text-2xl font-bold">
                    {prizes.reduce((sum, prize) => sum + prize.prize_value, 0).toLocaleString()}đ
                  </div>
