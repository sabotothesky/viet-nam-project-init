...
const RankRequestCard = ({ rankRequest, onUpdate, isClubOwner }: RankRequestCardProps) => {
  ...
  const getStatusColor = (status: RankRequest['status']) => {
    switch (status) {
      ...
      case 'on_site_test': return 'bg-blue-100 text-blue-800';
  ...
  const getStatusIcon = (status: RankRequest['status']) => {
    switch (status) {
      ...
      case 'on_site_test': return <AlertTriangle className="w-4 h-4" />;
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <CardTitle className="text-lg">
                {user?.nickname || user?.full_name || 'Người dùng không xác định'}
              </CardTitle>
              <p className="text-sm text-gray-600">
                {rankRequest.user?.email}
              </p>
            </div>
          </div>
