      ...
      <main className='container mx-auto px-4 py-8 pt-24'>
        <div className='mb-8'>
          ...
          <p className='text-xl text-gray-600'>
            Xếp hạng các cao thủ bida hàng đầu Việt Nam
          </p>
        </div>
        ...
        <div className='grid md:grid-cols-3 gap-6 mb-12'>
          {players?.slice(0, 3).map((player, index) => (
            <Card
              ...
              className={`relative overflow-hidden ${index === 0 ? 'md:order-2 transform md:scale-110' : index === 1 ? 'md:order-1' : 'md:order-3'}`}
            >
              <div
                className={`absolute top-0 left-0 right-0 h-2 ${index === 0 ? 'bg-yellow-400' : index === 1 ? 'bg-gray-400' : 'bg-amber-600'}`}
              ></div>
              ...
              <CardContent className='text-center space-y-2'>
                ...
                <div className='text-sm text-gray-600 space-y-1'>
                  <div>
                    Thắng: {player.wins} | Thua: {player.losses}
                  </div>
                  <div>
                    Tỷ lệ thắng:{' '}
                    {(
                      (player.wins / Math.max(player.total_matches, 1)) *
                      100
                    ).toFixed(1)}
                    %
                  </div>
          ...
          <CardContent>
            <div className='space-y-2'>
              {players?.map((player, index) => (
                <div
                  key={player.full_name}
                  className={`flex items-center justify-between p-4 rounded-lg transition-colors ${index < 3 ? 'bg-gradient-to-r from-yellow-50 to-orange-50 border border-yellow-200' : 'hover:bg-gray-50'}`}
                >
