...
export const ChatWindow: React.FC<ChatWindowProps> = ({
  ...
  const getConversationTitle = () => {
    ...
    if (conversation.type === 'group') {
      return (
        conversation.title ||
        `Nhóm ${conversation.participants.map(p => p.username).join(', ')}`
      );
    }
  ...
  const renderMessage = (message: ChatMessage) => {
    ...
    return (
      ...
      >
        <div
          className={`flex items-end gap-2 max-w-xs lg:max-w-md ${isOwnMessage ? 'flex-row-reverse' : 'flex-row'}`}
        >
          {!isOwnMessage && (
          ...
          >
            ...
            <div
              className={`flex items-center gap-1 mt-1 ${isOwnMessage ? 'flex-row-reverse' : 'flex-row'}`}
            >
