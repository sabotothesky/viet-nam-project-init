    ...
    <form onSubmit={handleSubmit(handleUpdateProfile)} className="space-y-4">
      <div>
        ...
        <Input
          ...
          {...register('nickname', { 
            ...
            minLength: { value: 2, message: 'Nickname phải có ít nhất 2 ký tự' },
            maxLength: { value: 20, message: 'Nickname không được quá 20 ký tự' }
          })}
          placeholder="Biệt danh hiển thị chính trong hệ thống"
        />
        {errors.nickname && (
          <p className="text-sm text-red-600 mt-1">{String(errors.nickname.message)}</p>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          />
          {errors.full_name && (
            <p className="text-sm text-red-600 mt-1">{String(errors.full_name.message)}</p>
          )}
        </div>
        <div>
