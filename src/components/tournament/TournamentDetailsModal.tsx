...
import { Calendar, MapPin, Users, Trophy, Clock, Phone, Mail, Loader2 } from 'lucide-react';
              ...
              <div>
                ...
                <div className="bg-gray-50 rounded-lg p-3 space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold">{getTournamentTypeLabel(tournament.tournament_type || 'single_elimination')}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Thể thức:</span>
                    <span className="font-semibold">{getGameFormatLabel(tournament.game_format || '8_ball')}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Số người tối đa:</span>
                    <span className="font-semibold">{tournament.max_participants} người</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Đã đăng ký:</span>
                    <span className="font-semibold text-blue-600">{tournament.current_participants || 0} người</span>
                  </div>
                </div>
              </div>
                ...
                <div className="bg-gray-50 rounded-lg p-3 space-y-2 text-sm">
                  {tournament.registration_start && (
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold">{formatDateTime(tournament.registration_start)}</span>
                    </div>
                  )}
                  {tournament.registration_end && (
                    <div className="flex justify-between">
                      <span className="text-gray-600">Đóng đăng ký:</span>
                      <span className="font-semibold">{formatDateTime(tournament.registration_end)}</span>
                    </div>
                  )}
                  <div className="flex justify-between">
                    <span className="text-gray-600">Bắt đầu thi đấu:</span>
                    <span className="font-semibold text-green-600">{formatDateTime(tournament.tournament_start || tournament.start_date)}</span>
                  </div>
                  {tournament.tournament_end && (
                    <div className="flex justify-between">
                      <span className="text-gray-600">Kết thúc dự kiến:</span>
                      <span className="font-semibold">{formatDateTime(tournament.tournament_end)}</span>
                    </div>
                  )}
                </div>
              </div>
                ...
                <div className="bg-gray-50 rounded-lg p-3">
                  <div className="flex items-start space-x-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-600 mt-1">{tournament.venue_address || tournament.club?.address}</p>
                ...
                <div className="bg-gradient-to-r from-yellow-50 to-orange-50 rounded-lg p-3">
                  <div className="grid grid-cols-3 gap-3 text-center">
                    <div className="bg-white rounded-lg p-3">
                      ...
                      <div className="font-bold text-yellow-600">{tournament.first_prize?.toLocaleString() || 0} điểm</div>
                    </div>
                    <div className="bg-white rounded-lg p-3">
                      <div className="text-2xl mb-1">🥈</div>
                      <div className="text-xs text-gray-600 mb-1">Giải nhì</div>
                      <div className="font-bold text-gray-600">{tournament.second_prize?.toLocaleString() || 0} điểm</div>
                    </div>
                    <div className="bg-white rounded-lg p-3">
                      <div className="text-2xl mb-1">🥉</div>
                      <div className="text-xs text-gray-600 mb-1">Giải ba</div>
                      <div className="font-bold text-orange-600">{tournament.third_prize?.toLocaleString() || 0} điểm</div>
                    </div>
                  </div>
                  ...
                  {tournament.rules ? (
                    ...
                    <div className="space-y-3 text-sm">
                      <div className="flex items-start space-x-2">
                        ...
                        <span>Thể thức thi đấu: {getTournamentTypeLabel(tournament.tournament_type || 'single_elimination')}</span>
                      </div>
                      <div className="flex items-start space-x-2">
                        <span className="text-blue-500 font-bold">2.</span>
                        <span>Loại bida: {getGameFormatLabel(tournament.game_format || '8_ball')}</span>
                      </div>
                      <div className="flex items-start space-x-2">
                        <span className="text-blue-500 font-bold">3.</span>
                        <span>Phí tham gia: {tournament.entry_fee_points || tournament.entry_fee || 0} điểm ranking</span>
                      </div>
                      <div className="flex items-start space-x-2">
                        <span className="text-blue-500 font-bold">4.</span>
                        <span>Người chơi phải có mặt trước giờ thi đấu 15 phút</span>
                      </div>
                      <div className="flex items-start space-x-2">
                        <span className="text-blue-500 font-bold">5.</span>
                        <span>Quyết định của trọng tài là quyết định cuối cùng</span>
                      </div>
                      <div className="flex items-start space-x-2">
                        <span className="text-blue-500 font-bold">6.</span>
                        <span>Không được sử dụng điện thoại trong khi thi đấu</span>
                      </div>
                    </div>
                  )}
                </div>
