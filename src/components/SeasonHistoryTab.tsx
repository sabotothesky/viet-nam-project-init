...
import { Search, Filter, Trophy, Users, TrendingUp, Target, User, Calendar } from 'lucide-react';
...
export const SeasonHistoryTab: React.FC<SeasonHistoryTabProps> = ({ className }) => {
  const { 
    loading, 
    error, 
    getSeasonHistory, 
  ...
  const [availableSeasons, setAvailableSeasons] = useState<Array<{ season_name: string; season_year: number }>>([]);
  ...
  const loadSeasonHistory = async () => {
    try {
      const response = await getSeasonHistory(filters, currentPage, itemsPerPage);
    ...
    <div className={`space-y-6 ${className}`}>
      <Tabs defaultValue="current" className="w-full">
        <TabsList className="grid w-full grid-cols-3">
          ...
          <TabsTrigger value="player-search" className="flex items-center gap-2">
                ...
                <CardContent className="p-6">
                  <div className="flex items-center space-x-2">
                    ...
                    <div>
                      ...
                      <p className="text-2xl font-bold">{formatPoints(stats.highest_points)}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardContent className="p-6">
                  <div className="flex items-center space-x-2">
                    ...
                    <div>
                      <p className="text-sm font-medium text-gray-600">Điểm thấp nhất</p>
                      <p className="text-2xl font-bold">{formatPoints(stats.lowest_points)}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium">Tìm kiếm theo nickname</label>
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-gray-600">
                    Hiển thị {((currentPage - 1) * itemsPerPage) + 1} - {Math.min(currentPage * itemsPerPage, totalCount)} 
                  ...
                  <tbody>
                    {history.map((player, index) => (
                      <tr key={player.id} className="border-b hover:bg-gray-50">
                        <td className="p-3">
                          <Badge className={getRankBadgeColor(player.final_rank)}>
                            #{player.final_rank}
                          </Badge>
                        </td>
