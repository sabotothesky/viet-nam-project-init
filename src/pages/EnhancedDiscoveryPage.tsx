...
import { ArrowLeft, Settings, X, Star, Zap, Filter, RefreshCw, Users, Trophy, MapPin } from 'lucide-react';
...
const EnhancedDiscoveryPage = () => {
  ...
  const handleSendChallenge = async (challengeData: { betPoints: number; message: string }) => {
              ...
              {nearbyPlayers.length === 0 ? (
                ...
                >
                  <div className="bg-white rounded-2xl p-8 text-center shadow-2xl">
                    ...
                    <h3 className="text-xl font-bold text-gray-800 mb-2">Không có đối thủ</h3>
                    <p className="text-gray-600 mb-4">Hiện tại không có người chơi phù hợp</p>
                ...
                nearbyPlayers.slice(currentIndex, currentIndex + 3).map((player, index) => (
                  <motion.div
                  ...
                  >
                    <EnhancedPlayerCard 
                      player={player} 
                      index={index}
                      onChallenge={index === 0 ? () => handleSwipe('right', player) : undefined}
                    />
                  </motion.div>
                ))
              )}
