...
const GoogleMapsPlacesAutocomplete = ({ onPlaceSelect, disabled = false }: GoogleMapsPlacesAutocompleteProps) => {
  const [query, setQuery] = useState('');
  const [predictions, setPredictions] = useState<GooglePlacesResponse['predictions']>([]);
  ...
  const handlePredictionClick = (prediction: GooglePlacesResponse['predictions'][0]) => {
      ...
      <div className="space-y-2">
        ...
        <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-md">
          <p className="text-sm text-yellow-800">
            Vui lòng cấu hình VITE_GOOGLE_PLACES_KEY trong file .env để sử dụng tính năng tìm kiếm CLB trên Google Maps.
          </p>
        </div>
      </div>
    );
        ...
        <div className="mt-3 p-3 bg-green-50 border border-green-200 rounded-md">
          <div className="flex items-start space-x-2">
            ...
            <div className="flex-1">
              ...
              <div className="text-sm text-green-700">{selectedPlace.formatted_address}</div>
              <div className="text-xs text-green-600 mt-1">
                Tọa độ: {selectedPlace.geometry.location.lat.toFixed(6)}, {selectedPlace.geometry.location.lng.toFixed(6)}
              </div>
            </div>
          </div>
        </div>
