...
const OpponentCard = ({ opponent, onSwipeLeft, onSwipeRight, onChallenge, onViewProfile }: OpponentCardProps) => {
        ...
        <div className="pt-10 px-4 pb-4">
          ...
          <div className="mb-4">
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-xl font-bold text-gray-900">{opponent.name}</h3>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4 p-3 bg-tertiary rounded-lg">
            <div className="text-center">
              ...
              <p className="text-lg font-bold text-primary-blue">{opponent.stats.matches_played}</p>
            </div>
            <div className="text-center">
              <p className="text-xs text-comment-gray">Thắng</p>
              <p className="text-lg font-bold text-primary-green">{opponent.stats.matches_won}</p>
            </div>
            <div className="text-center">
              <p className="text-xs text-comment-gray">Tỉ lệ thắng</p>
              <p className={`text-lg font-bold ${getWinRateColor(opponent.stats.win_rate)}`}>
                {opponent.stats.win_rate}%
              </p>
            </div>
            <div className="text-center">
              <p className="text-xs text-comment-gray">Run dài nhất</p>
              <p className="text-lg font-bold text-gold">{opponent.stats.longest_run}</p>
            </div>
          </div>
          ...
          {opponent.preferred_stakes && (
            <div className="mb-4">
              <p className="text-xs text-comment-gray mb-2">Mức cược ưa thích:</p>
