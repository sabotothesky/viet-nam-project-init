    ...
    try {
      ...
      const clubStats = clubIds.map(clubId => {
        ...
        return {
          ...
          total_elo_points: clubData.reduce(
            (sum, s) => sum + s.total_elo_points,
            0
          ),
          average_elo_points: clubData.length
            ? clubData.reduce((sum, s) => sum + s.total_elo_points, 0) /
              clubData.length
            : 0,
          top_player: clubData.sort(
            (a, b) => b.total_elo_points - a.total_elo_points
          )[0],
        };
      });
