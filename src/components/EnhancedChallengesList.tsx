...
import { Clock, CheckCircle, XCircle, Trophy, Calendar, MapPin } from 'lucide-react';
...
const EnhancedChallengesList = () => {
  ...
  const handleRespond = async (status: 'accepted' | 'declined', proposalData?: ChallengeProposal) => {
    ...
    <div className="space-y-4">
      <Tabs defaultValue="received" className="w-full">
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="received">Nhận được ({receivedChallenges.length})</TabsTrigger>
          <TabsTrigger value="sent">Đã gửi ({sentChallenges.length})</TabsTrigger>
        </TabsList>
            ...
            receivedChallenges.map((challenge) => (
              <Card key={challenge.id} className="overflow-hidden">
                <CardHeader className="pb-3">
                  <div className="flex justify-between items-start">
                    <div className="flex items-center space-x-3">
                      ...
                      />
                      <div>
                        <CardTitle className="text-lg">{challenge.challenger.full_name}</CardTitle>
                ...
                <CardContent className="space-y-4">
                  ...
                  {challenge.status === 'accepted' && challenge.proposed_datetime && (
                    <div className="bg-green-50 rounded-lg p-3">
                      <div className="flex items-center space-x-2 mb-2">
                        <Calendar className="w-4 h-4 text-green-600" />
                        <span className="text-sm font-semibold text-green-800">Lịch đề xuất</span>
                      </div>
                    ...
                    <div className="flex items-center space-x-3">
                      <UserAvatar 
                        user={{
                          ...
                          avatar: challenge.challenged.avatar_url || '/placeholder.svg',
                          rank: challenge.challenged.current_rank
                        }}
                        size="md"
                      />
                      <div>
                        <CardTitle className="text-lg">{challenge.challenged.full_name}</CardTitle>
                        <p className="text-sm text-gray-600">
