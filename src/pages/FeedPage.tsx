...
import { Plus, Filter, Search, Users, Trophy, MapPin, Bell } from 'lucide-react';
...
const FeedPage = () => {
  ...
  const handleChallengeAction = (action: string, challengeId: string) => {
    toast.success(`Thách đấu đã được ${action === 'accepted' ? 'chấp nhận' : 'từ chối'}`);
  };
  ...
  const filteredChallenges = receivedChallenges.filter(challenge =>
    challenge.challenger_profile?.full_name?.toLowerCase().includes(searchQuery.toLowerCase())
  );
        ...
        <div className="bg-white p-4 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-sm text-gray-600">Cập nhật mới nhất từ cộng đồng</p>
            </div>
