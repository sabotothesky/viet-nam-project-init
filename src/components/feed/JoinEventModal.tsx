      ...
      return (
        <div className="space-y-4">
          <div className="text-center">
            ...
            <h2 className="text-xl font-bold text-gray-900 mb-2">{post.event.title}</h2>
            ...
            <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
              ...
              <div>
                ...
                <div className="text-sm text-gray-600">
                  {format(post.event.date, 'EEEE, dd/MM/yyyy HH:mm', { locale: vi })}
                </div>
                <div className="text-xs text-gray-500">
                  {formatDistanceToNow(post.event.date, { addSuffix: true, locale: vi })}
                </div>
              </div>
            </div>
          ...
          {post.event.is_joined && (
            <div className="flex items-center gap-2 p-3 bg-green-50 border border-green-200 rounded-lg">
              ...
              <span className="text-green-800 font-medium">Bạn đã tham gia sự kiện này</span>
            </div>
          )}

          {post.event.max_participants && post.event.participants_count >= post.event.max_participants && (
            <div className="flex items-center gap-2 p-3 bg-red-50 border border-red-200 rounded-lg">
              <XCircle className="h-5 w-5 text-red-600" />
              <span className="text-red-800 font-medium">Sự kiện đã đầy người tham gia</span>
            </div>
          )}
        </div>
      );
      ...
      return (
        <div className="space-y-4">
          <div className="text-center">
            ...
            <h2 className="text-xl font-bold text-gray-900 mb-2">{post.challenge.title}</h2>
            ...
            {post.challenge.opponent && (
              <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                ...
                <div>
                  ...
                  <div className="text-sm text-gray-600">{post.challenge.opponent.username}</div>
                </div>
              </div>
            )}
          ...
          {post.challenge.status === 'accepted' && (
            <div className="flex items-center gap-2 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              ...
              <span className="text-blue-800 font-medium">Thách đấu đã được chấp nhận</span>
            </div>
          )}
          ...
          {post.challenge.status === 'completed' && (
            <div className="flex items-center gap-2 p-3 bg-gray-50 border border-gray-200 rounded-lg">
              ...
              <span className="text-gray-800 font-medium">Thách đấu đã hoàn thành</span>
            </div>
          )}
        </div>
      );
      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            {post.type === 'event' ? <Calendar className="h-5 w-5" /> : <Target className="h-5 w-5" />}
